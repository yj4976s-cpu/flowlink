...
Copilot chat rate limit은 GUEST의 ASGI peer IP와 인증된 USER/ADMIN ID를 기준으로 구분합니다. 기본 구현은 process-local in-memory 방식이므로 multi-worker 배포에서는 worker 간 제한이 공유되지 않으며, 운영 규모가 커지면 `copilot_rate_limit.py` 경계에서 공유 저장소 기반 구현으로 교체해야 합니다.
