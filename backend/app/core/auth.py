...
def ensure_user(user: User) -> User:
    if user.role != "USER":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User permission required")
    return user
...
def get_optional_current_user(request: Request, db: Annotated[Session, Depends(get_db)]) -> User | None:
    ...
    try:
        payload = decode_access_token(token)
        return ensure_active_user(get_user_by_id(db, int(payload["sub"])))
...
def require_user(current_user: Annotated[User, Depends(get_current_user)]) -> User:
    return ensure_user(current_user)
