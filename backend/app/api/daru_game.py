...
def history(current_user: Annotated[User, Depends(require_user)], db: Annotated[Session, Depends(get_db)], difficulty: Annotated[Difficulty, Query()] = "EASY", page: Annotated[int, Query(ge=1)] = 1, page_size: Annotated[int, Query(ge=1, le=20)] = 5) -> DaruGameHistoryResponse:
    ...
    items = [DaruGameHistoryItem(id=item.id, difficulty=item.difficulty, detection_power=float(item.detection_power), attempts=item.attempts, elapsed_seconds=item.elapsed_seconds, max_combo=item.max_combo, hints_used=item.hints_used, earned_daru_points=item.earned_daru_points, completed=item.completed, within_time_limit=item.within_time_limit, achieved_at=item.achieved_at, deleted_at=None, is_best=bool(stat and item.completed and stat.ranking_record_id == item.id), is_ranking_record=bool(stat and stat.ranking_record_id == item.id)) for item in records]
