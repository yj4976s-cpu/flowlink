...
for (const [theme, themeConfig] of Object.entries(THEMES)) {
  ...
  for (let y = 0; y < source.info.height; y += 1) {
    for (let x = 0; x < source.info.width; x += 1) {
      ...
      const masked = isScarfBlue(red, green, blue, alpha, x, y, source.info.width, source.info.height);
      if (!masked) {
        const [hue, saturation] = rgbToHsl(red, green, blue);
        if (alpha > 0 && hue >= 195 && hue <= 250 && saturation >= 0.22) preservedBluePixels += 1;
        continue;
      }
  ...
  const outputPath = path.join(MASCOT_ROOT, `daru-idle-${theme}.png`);
  ...
  for (let y = 0; y < source.info.height; y += 1) {
    for (let x = 0; x < source.info.width; x += 1) {
      ...
      const rgbChanged = source.data[offset] !== generated.data[offset]
        || source.data[offset + 1] !== generated.data[offset + 1]
        || source.data[offset + 2] !== generated.data[offset + 2];
      if (rgbChanged && !masked) unexpectedRgbDifferences += 1;
    }
  }

  if (
    generated.info.width !== source.info.width
    || generated.info.height !== source.info.height
    || alphaDifferences > 0
    || unexpectedRgbDifferences > 0
    || changedPixels === 0
    || preservedBluePixels === 0
  ) {
    throw new Error(`Validation failed for ${theme} production idle`);
  }
