...
function mapReport(report: ApiReport): CitizenReport {
  ...
  return { id: String(report.id), category, title: `${report.color ?? ""} ${category}`.trim(), color: report.color ?? "",
    ...
    status: statusLabels[report.status] ?? "검토 대기", statusCode: report.status as CitizenReportStatusCode, mapPosition: { x: 24 + report.id % 55, y: 28 + report.id % 38 },
...
export async function deleteCitizenReport(reportId: string) { return mapReport(await request<ApiReport>(`/api/citizen-reports/${reportId}`, { method: "DELETE" })); }
