...
export function PartialResult({ matchedPairs, pairCount, maxCombo, daruPoints, elapsedSeconds, recordStatus, onRestart, onChangeDifficulty, startPending = false }: { matchedPairs: number; pairCount: number; maxCombo: number; daruPoints: number; elapsedSeconds: number; recordStatus: ResultRecordStatus; onRestart: () => void; onChangeDifficulty: () => void; startPending?: boolean }) {
  ...
  return <section ref={ref} className={`${styles.result} ${styles.partialResult}`} role="dialog" aria-modal="true" aria-labelledby="partial-result-title" tabIndex={-1}>
    ...
    <div className={styles.resultActions}><button className="button button-primary" type="button" disabled={startPending} aria-busy={startPending || undefined} onClick={onRestart}>{startPending ? "게임 준비 중…" : "한 번 더 찾기"}</button><button className="button button-secondary" type="button" disabled={startPending} onClick={onChangeDifficulty}>다른 난이도<br />도전하기</button></div>
  </section>;
}
