    ...
    <section className={styles.selector} aria-labelledby="daru-game-title">
      ...
      <div className={styles.previewBoard} aria-label="카드 뒤집기 게임 미리보기">
        ...
        {(["umbrella", "shoe"] as const).map((cardId, index) => {
          ...
          return <button key={cardId} className={`${styles.previewCard} ${styles.previewBack} ${index ? styles.previewTwo : styles.previewOne}`} type="button" aria-label={`${card.label} 미리보기 카드 뒤집기`} aria-pressed={previewCard === cardId} data-memory-preview-card="true" data-revealed={previewCard === cardId || undefined} onClick={() => revealPreview(cardId)}><span className={styles.previewCardInner}><span className={styles.previewCardBack}><span className={styles.cardBackArtwork}><Image key={theme} src={DARU_CARD_BACK_ASSETS[theme]} alt="" fill sizes="82px" draggable={false} unoptimized /></span></span><span className={`${styles.previewCardFront} ${styles.previewCardShell}`}><Image src={getCardThemeImages(card)[theme]} alt="" fill sizes="82px" /></span></span></button>;
        })}
        <div className={styles.previewDaru} data-memory-preview-daru="true" aria-hidden="true"><Image className={styles.daruDawn} src="/mascot/daru-idle-dawn.png" alt="" fill sizes="180px" priority /><Image className={styles.daruDay} src="/mascot/daru-idle-day.png" alt="" fill sizes="180px" priority /><Image className={styles.daruNight} src="/mascot/daru-idle-night.png" alt="" fill sizes="180px" priority /></div>
        <p className={styles.previewBubble} data-memory-preview-bubble="true"><strong>준비됐어?</strong><span>같이 시작해볼까?</span></p>
        {PREVIEW_ITEMS.map((card, index) => <div className={`${styles.previewCard} ${styles.previewFace} ${styles.previewCardShell} ${styles[`previewItem${index + 1}`]}`} data-memory-preview-card="true" key={card.id} aria-hidden="true"><Image src={getCardThemeImages(card)[theme]} alt="" fill sizes="82px" /></div>)}
