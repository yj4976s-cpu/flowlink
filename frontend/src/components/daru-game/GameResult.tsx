...
export function GameResult({ rank, metrics, daruPoints, elapsedSeconds, attempts, maxCombo, hintsUsed, withinTimeLimit, newBest, leaderboardRank, personalBestPower, recordStatus, difficultyLabel, onRestart, onChangeDifficulty, onViewLeaderboard, previewTheme }: { rank: GameRank; metrics: DetectionMetrics; daruPoints: number; elapsedSeconds: number; attempts: number; maxCombo: number; hintsUsed: number; withinTimeLimit: boolean; newBest: boolean; leaderboardRank: number | null; personalBestPower?: number | null; recordStatus: ResultRecordStatus; difficultyLabel: string; onRestart: () => void; onChangeDifficulty: () => void; onViewLeaderboard?: () => void; previewTheme?: DaruGameTheme }) {
  ...
  return <section ref={stageRef} className={styles.result} role="dialog" aria-modal="true" aria-labelledby="game-result-title" tabIndex={-1}>
    ...
    <h2 id="game-result-title">다루와 전부 찾았어요!</h2><p className={styles.rankMessage}>{withinTimeLimit ? RANK_MESSAGES[rank] : "시간 초과 후 완주했어요. 공식 랭킹에는 반영되지 않아요."}</p>
    ...
    {newBest && <p className={styles.newRecord}>✨ 새로운 최고 기록!</p>}
