...
export function DaruLeaderboard({ refreshKey = 0, preview }: { refreshKey?: number; preview?: LeaderboardPreview }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      else if (deleteTarget === "all" || deleteTarget === "difficulty") setDeleteSuccess(deletedCount === 0 ? "정리할 수 있는 기록이 없어요." : `플레이 기록 ${deletedCount}개를 휴지통으로 이동했어요.`);
      ...
      setDeleteError("요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.");
    } finally { setDeleting(false); }
  };
  ...
  return <section id="daru-leaderboard" className={styles.leaderboard} aria-labelledby="leaderboard-title" aria-busy={loading}>
    <header className={styles.leaderboardHeader}>
      <div><h2 id="leaderboard-title">🏆 다루 메모리 랭킹</h2><p>기억력 · 속도 · 콤보 · 힌트로 결정돼요 · 난이도별 개인 최고 기록이 랭킹에 반영돼요</p></div>
