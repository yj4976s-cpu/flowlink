...
export function DaruLeaderboard({ refreshKey = 0, preview }: { refreshKey?: number; preview?: LeaderboardPreview }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      } else if (deleteTarget === "all") await deleteAllDaruGameHistory();
      ...
      else setDeleteSuccess(deleteTarget === "all" ? "모든 난이도의 플레이 기록을 휴지통으로 이동했습니다." : `플레이 기록 ${deletedCount}개를 휴지통으로 이동했습니다.`);
