...
type ReviewFilter = "ALL" | "PENDING" | "CONFIRMED" | "REJECTED" | "WASTE_PENDING" | "WASTE_COMPLETED";
...
function eventMatchesFilter(event: DetectionEvent, filter: ReviewFilter) {
  if (filter === "WASTE_PENDING") return eventHasWastePending(event);
  if (filter === "WASTE_COMPLETED") return eventHasWasteCompleted(event);
  return filter === "ALL" || eventReview(event) === filter || (filter === "REJECTED" && eventReview(event) === "EMPTY");
}
function defaultObjectForFilter(event: DetectionEvent, filter: ReviewFilter) {
  if (filter === "WASTE_PENDING") return event.detected_objects.find(isWastePendingObject) ?? null;
  if (filter === "WASTE_COMPLETED") return event.detected_objects.find(isWasteCompletedObject) ?? null;
  return event.detected_objects[0] ?? null;
}
function objectMatchesFilter(object: DetectionObject, filter: ReviewFilter) {
  ...
  if (filter === "WASTE_COMPLETED") return isWasteCompletedObject(object);
  return true;
}
...
function ObjectReview({ object, event, onSaved }: { object: DetectionObject; event: DetectionEvent; onSaved: (object: DetectionObject) => void }) {
  ...
  const completed = object.follow_up_kind === "FOUND_ITEM" ? object.found_item_id !== null : object.follow_up_kind === "WASTE" ? object.waste_collection_completed : false; const availableAction = object.follow_up_kind === "FOUND_ITEM" || object.follow_up_kind === "WASTE" ? object.follow_up_kind : null; const followUpReady = canRunDetectionFollowUp(object); const followUpLocked = object.found_item_id !== null || object.waste_collection_completed;
  return <div className={styles.inspectorContent}><div className={styles.objectSummary}><Media compact src={object.cropped_image_url} fallbackSrc={eventImage(event)} alt={`${object.object_class_name} 탐지 객체`} /><div><span>객체 #{object.id}</span><strong>{classLabel(object)}</strong><b>AI 신뢰도 {Math.round(Number(object.confidence) * 100)}%</b><small>bbox {Number(object.bbox_x).toFixed(0)}, {Number(object.bbox_y).toFixed(0)} · {Number(object.bbox_width).toFixed(0)}×{Number(object.bbox_height).toFixed(0)}</small></div></div><div className={styles.reviewForm}><SelectBox label="최종 분류" value={finalClass} options={classOptions} onChange={setFinalClass} disabled={saving || processing || followUpLocked} /><div className={styles.colorReview}><div><span>AI 추정 색상</span><strong>{event.purpose === "USER_ANALYSIS" ? "사용자 분석에서는 미적용" : object.ai_color ?? "추정하지 못함"}</strong></div><p>{event.purpose === "USER_ANALYSIS" ? "색상 추정은 운영 이미지 탐지에만 적용됩니다." : "이미지와 조명에 따라 다를 수 있어요. 관리자가 확인한 값이 우선 적용됩니다."}</p></div><label className={styles.field}><span>최종 확인 색상</span><select value={confirmedColor} onChange={(event) => setConfirmedColor(event.target.value)} disabled={saving || processing}><option value="">확정하지 않음 · AI 추정값 사용</option>{itemColorOptions.map((color) => <option value={color} key={color}>{color}</option>)}</select></label><SelectBox label="처리 상태" value={status} options={statusOptions} onChange={(value) => setStatus(value as DetectionObject["processing_status"])} disabled={saving || processing || followUpLocked} /><label className={styles.field}><span>관리자 메모</span><textarea value={memo} onChange={(event) => setMemo(event.target.value)} disabled={saving || processing} placeholder="검토 근거 또는 참고 내용을 입력하세요." /></label>{message && <p className={styles.success} role="status">{message}</p>}{error && <p className={styles.error} role="alert">{error}</p>}<div className={styles.saveBar}><button className="button button-primary" type="button" disabled={saving || processing} onClick={() => void save()}>{saving ? "저장 중" : "검토 내용 저장"}</button></div></div><section className={styles.followUp}><h3>후속 처리</h3>{event.purpose === "USER_ANALYSIS" ? <div className={styles.analysisNotice}><Icon name="userSearch" size={19} /><div><strong>사용자 분석 결과입니다</strong><p>이 기록 자체는 운영 후속 처리 대상이 아닙니다. 사용자가 발견 제보를 접수한 경우 <Link href="/admin/citizen-reports">제보 관리</Link>에서 확인하세요.</p></div></div> : !followUpReady ? <p>검토 상태를 저장한 후 후속 처리를 진행할 수 있습니다.</p> : !availableAction ? <p>이 객체에는 가능한 후속 처리가 없습니다.</p> : completed ? <div className={styles.followUpComplete}><Icon name="packageCheck" size={19} /><strong>{object.follow_up_kind === "FOUND_ITEM" ? "공식 발견물 등록 완료" : "수거 완료"}</strong>{object.found_item_id && <Link href={`/found-items/${object.found_item_id}`}><Icon name="fileSearch" size={17} />발견물 확인</Link>}</div> : <button type="button" className="button button-secondary" disabled={processing} onClick={() => setConfirming(availableAction)}><Icon name={availableAction === "FOUND_ITEM" ? "archive" : "packageCheck"} size={18} />{availableAction === "FOUND_ITEM" ? "공식 발견물로 등록" : "수거 완료 처리"}</button>}</section>{confirming && <ConfirmDialog title={confirming === "FOUND_ITEM" ? "공식 발견물로 등록할까요?" : "수거 완료로 처리할까요?"} description={confirming === "FOUND_ITEM" ? "최종 확인 색상을 우선 적용해 공식 발견물을 생성합니다." : "처리 후 수거 완료 이력이 기록됩니다."} confirmLabel={confirming === "FOUND_ITEM" ? "등록하기" : "처리 완료"} busy={processing} onCancel={() => setConfirming(null)} onConfirm={() => void followUp()} />}</div>;
}
  ...
  return <div className={styles.focusBackdrop} onMouseDown={(mouseEvent) => mouseEvent.target === mouseEvent.currentTarget && onClose()}>
    <div className={styles.focusDialog} role="dialog" aria-modal="true" aria-labelledby="focused-review-title" ref={dialogRef}>
      ...
      <div className={styles.focusBody}><section className={styles.focusMediaPanel} aria-label="탐지 이미지 상세 보기"><div className={styles.focusViewport} data-draggable={zoom > 100} ref={viewportRef} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={(wheelEvent) => { if (!wheelEvent.ctrlKey) return; wheelEvent.preventDefault(); changeZoom(zoom + (wheelEvent.deltaY < 0 ? REVIEW_ZOOM_STEP : -REVIEW_ZOOM_STEP)); }}>
        ...
        </> : <div className={styles.focusMediaEmpty}><Icon name="scanLine" size={46} /><span>표시할 탐지 이미지가 없습니다.</span></div>}
      ...
      <aside className={styles.focusInspector} aria-label="선택 객체 검토">{object ? <ObjectReview key={object.id} object={object} event={event} onSaved={onSaved} /> : <div className={styles.inspectorEmpty}>검토할 객체가 없습니다.</div>}</aside></div>
    </div>
  </div>;
}

export function AdminDetectionsClient() {
  const queryParams = useSearchParams(); const requested = Number(queryParams.get("detection")); const requestedStatus = queryParams.get("status"); const requestedPurpose = queryParams.get("purpose"); const requestedFollowUp = queryParams.get("followUp"); const requestedFilter: ReviewFilter = requestedFollowUp === "WASTE_PENDING" ? "WASTE_PENDING" : requestedFollowUp === "WASTE_COMPLETED" ? "WASTE_COMPLETED" : requestedStatus === "PENDING" ? "PENDING" : "ALL"; const [events, setEvents] = useState<DetectionEvent[]>([]); const [selectedId, setSelectedId] = useState<number | null>(null); const [selectedObjectId, setSelectedObjectId] = useState<number | null>(null); const [filter, setFilter] = useState<ReviewFilter>(requestedFilter); const [purpose, setPurpose] = useState(requestedPurpose === "OPERATION" ? "OPERATION" : "ALL"); const [sort, setSort] = useState("NEWEST"); const [search, setSearch] = useState(""); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  ...
  const load = () => { const controller = new AbortController(); setLoading(true); setError(""); listAdminDetections(controller.signal).then(applyData).catch((reason) => { if (!isAbortError(reason)) setError("탐지 결과를 불러오지 못했습니다."); }).finally(() => { if (!controller.signal.aborted) setLoading(false); }); return controller; };
  useEffect(() => { const controller = new AbortController(); listAdminDetections(controller.signal).then(applyData).catch((reason) => { if (!isAbortError(reason)) setError("탐지 결과를 불러오지 못했습니다."); }).finally(() => { if (!controller.signal.aborted) setLoading(false); }); return () => controller.abort(); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { const controller = new AbortController(); listAdminCameras(controller.signal).then((rows) => { setCameras(rows); setCameraId(String(rows[0]?.id ?? "")); }).catch((reason) => { if (!isAbortError(reason)) setOperationError("운영 카메라를 불러오지 못했습니다."); }); return () => controller.abort(); }, []);
  const counts = useMemo(() => ({ ALL: events.length, PENDING: events.filter((event) => eventReview(event) === "PENDING").length, CONFIRMED: events.filter((event) => eventReview(event) === "CONFIRMED").length, REJECTED: events.filter((event) => ["REJECTED", "EMPTY"].includes(eventReview(event))).length, WASTE_PENDING: events.filter(eventHasWastePending).length, WASTE_COMPLETED: events.filter(eventHasWasteCompleted).length }), [events]);
  const visible = useMemo(() => events.filter((event) => eventMatchesFilter(event, filter) && (purpose === "ALL" || event.purpose === purpose) && (!search.trim() || String(event.id).includes(search.trim()) || event.detected_objects.some((object) => `${object.object_class_name} ${object.object_class} ${object.final_class_code ?? ""}`.toLowerCase().includes(search.trim().toLowerCase())))).sort((a, b) => (new Date(b.captured_at).getTime() - new Date(a.captured_at).getTime()) * (sort === "NEWEST" ? 1 : -1)), [events, filter, purpose, search, sort]);
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.summary} role="tablist" aria-label="탐지 검토 상태">{([{ value: "ALL", label: "전체" }, { value: "PENDING", label: "검토 필요" }, { value: "CONFIRMED", label: "검토 완료" }, { value: "REJECTED", label: "제외" }, { value: "WASTE_PENDING", label: "수거 대기" }, { value: "WASTE_COMPLETED", label: "수거 완료" }] as const).map((item) => <button type="button" role="tab" aria-selected={filter === item.value} data-status={item.value} onClick={() => setFilter(item.value)} key={item.value}><span>{item.label}</span><b>{counts[item.value]}</b></button>)}</div>
