...
export function CommunityPlaceSearch({ value, optional = false, onChange, onSelect }: { value: string; optional?: boolean; onChange: (value: string) => void; onSelect: (place: CommunityPlace | null) => void }) {
  ...
  useEffect(() => { const id = ++sequence.current; if (value.trim().length < 2) { const timer = window.setTimeout(() => { if (sequence.current !== id) return; setItems([]); setOpen(false); setLoading(false); }); return () => window.clearTimeout(timer); } const timer = window.setTimeout(() => { setLoading(true); void searchKakaoPlacesPage(value.trim()).then(({ places }) => { if (sequence.current === id) { setItems(places); setOpen(true); } }).catch(() => { if (sequence.current === id) setItems([]); }).finally(() => { if (sequence.current === id) setLoading(false); }); }, 300); return () => window.clearTimeout(timer); }, [value]);
  ...
  const choose = (item: KakaoPlace) => { sequence.current += 1; const place = { placeName: item.place_name, address: item.address_name, roadAddress: item.road_address_name || undefined, latitude: Number(item.y), longitude: Number(item.x) }; onChange(place.placeName); onSelect(place); setItems([]); setLoading(false); setOpen(false); };
  const changeQuery = (nextValue: string) => { sequence.current += 1; onChange(nextValue); onSelect(null); if (nextValue.trim().length < 2) { setItems([]); setLoading(false); setOpen(false); } };
  const clear = () => { sequence.current += 1; onChange(""); onSelect(null); setItems([]); setLoading(false); setOpen(false); };
  return <div className={styles.placeSearch} ref={root}><Icon name="location" size={18} /><input value={value} onChange={(event) => changeQuery(event.target.value)} onFocus={() => items.length && setOpen(true)} placeholder="장소명이나 지역명을 검색해 주세요" aria-label="커뮤니티 지역 검색" />{value && <button type="button" aria-label="지역 초기화" onClick={clear}><Icon name="close" size={15} /></button>}{open && <div className={styles.placeResults} role="listbox">{loading && <p>장소를 찾고 있어요.</p>}{!loading && items.map((item) => <button type="button" role="option" aria-selected="false" key={item.id} onClick={() => choose(item)}><strong>{item.place_name}</strong><small>{item.road_address_name || item.address_name}</small></button>)}</div>}{optional && <small>위치는 선택 사항이에요.</small>}</div>;
}
